...
import { LayoutDashboard, Users, ClipboardList, X, BarChart3, Calendar, DollarSign, FileText, ClipboardCheck, MessageSquare, BookOpen, Brain, Bot, ListChecks, FolderKanban } from "lucide-react";
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-64 flex-col transition-transform duration-300 md:relative md:translate-x-0 bg-[#a0181c]", open ? "translate-x-0" : "-translate-x-full")}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-4">
          {links.map((link) => {
            ...
            return (
              <NavLink key={link.to} to={link.to} onClick={onClose} className={cn("flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors", isActive ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground")}>
